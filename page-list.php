...
function g($refresh,$delete) {
	...
	if (!$pages && !$refresh && !$delete) {
		echo '<span id="none">You don\'t have any pages yet. Why not <a href="backend/new.php">create a new one?</a></span>';
	} else {
		foreach ($pages as $i) {
			$name = fname($i);
			if ($GLOBALS['gedit'] == true) {
				echo '<form action="backend/edit" method="POST"><button type="submit" name="page" value="' . $name. '" class="page button">' . $name . '</button></form>';
			}
			elseif ($GLOBALS['refresh'] == true) {
				if (strtolower(fname($i)) == "index") {
					$entry = '<li class="index"><a href="index.php" class="page">' . ucfirst($name) . '</a></li>';
				} else {
